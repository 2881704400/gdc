...
function message() {
    // 主页面下拉刷新
    var ptrContent = $$('#message_pg_content');
...
$$(document).on("pageBeforeInit", ".page[data-page='MessageList']", function (e) {
    ...
    var MessageList_rsh = $$('#MessageList_ref');
    myApp.initPullToRefresh(MessageList_rsh);
    MessageList_rsh.on("refresh", function (e) {
        setTimeout(function () {
            SnapshotData();
            // 加载完毕需要重置
            myApp.pullToRefreshDone(MessageList_rsh);
        }, 2000);
    })
});
...
function SysEvtCounts() {
    if ($$(".view-main").attr("data-page") != "message") {
        ...
        ajaxService("post", _url, false, _data, _successf, _errors);
    }
}
