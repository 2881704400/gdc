...
function jsonTobtn(data, confarr) {
    ...
    for (var i = 0; i < usera.length; i++) {
        ...
        var newRow = "<tr><td><button class=\"btn btns-style-4\" onclick=\"onSetClickBtn(" + confarr + ",'" + userc[1] + "','" + userc[2] + "','" + userc[3] + "','" + userc[0] + "')\">" + userc[0] + "</button></td></tr>";
...
function onSetClickBtn(str_1, str_2, str_3, str_4, btnName) {
    ...
    if (str_4 == "") {
        newContainer += "<p class='MessageBox_p2'><span>设置值：</span><input id='setValues' type='text' class='input' style='margin-top:6px;'/></p>";
    }
    else {
...
function onSetCommand(str_1, str_2, str_3, str_4, dt) {
    ...
    var _dataSet = "set_nm=" + str_1 + "&&main_instruction=" + str_2 + "&&minor_instruction=" + str_3 + "&&values=" + vals + "&&usern=" + $("#userName").text();
    JQajaxo("post", _url, false, _dataSet, _successfSet);
    function _successfSet(data) {
        var resultJs = $(data).children("string").text();
        if (resultJs != "false") {
